...
export default function PipelineBoard({ 
  ...
  // Group leads by their stageId using useMemo to prevent O(N*M) filtering on every render
  const leadsByStage = useMemo(() => {
    const grouped = new Map<string, any[]>();
    for (const lead of leads) {
      if (!grouped.has(lead.stageId)) {
        grouped.set(lead.stageId, []);
      }
      grouped.get(lead.stageId)!.push(lead);
    }
    return grouped;
  }, [leads]);
